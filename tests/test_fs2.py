...
class TestFS2(unittest.TestCase):
    ...
    def _realpath(self, path):
        _path = os.path.join(self.dir_fscmd, relpath(path))
        return _path

    def setUp(self):
        self.dir_fscmd = tempfile.mkdtemp("testfscmd")
    ...
    def tearDown(self):
        try:
            shutil.rmtree(self.dir_fscmd)
        except OSError:
            print(self.dir_fscmd, 'Already deleted')

    def test_help(self):
        result = self.runner.invoke(fscmd, ['--help'])
        print(result.output)
    ...
    def test_listopener(self):
        result = self.runner.invoke(fscmd, ['--listopener'])
        print(result.output)
        self.assertEqual(result.exit_code, 0)
    ...
    def test_ls(self):
        ...
        result = self.runner.invoke(fscmd, args, input='N\n')
        print(result.output)
    ...
    def test_cat(self):
        ...
        result = self.runner.invoke(fscmd, args, input='Y\nN\n')
        print(result.output)
        self.assertNotEqual(result.exit_code, 0)

    def test_tree(self):
        args = ['-u', self._realpath('/'), 'tree', 'a/']
        result = self.runner.invoke(fscmd, args)
        print(result.output)
        self.assertEqual(result.exit_code, 0)
        self.assertIn('a1.txt',  result.output)
